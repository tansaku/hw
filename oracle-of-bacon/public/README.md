...
1. Delete `,:pending => true` from the `describe 'instance'` block,
...
1. Define the constructor for OracleOfBacon so that both From and To
initially default to Kevin Bacon, rather than being blank.  This should
cause a subset of the validity specs to pass green.

2. Since it makes no sense to connect an actor to herself, we should
